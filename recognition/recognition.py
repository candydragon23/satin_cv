...
def count_lines(region):
    shape = region.image.shape
    vlines = (np.sum(image, 0) / image.shape[0] == 1).sum()
    hlines = (np.sum(image, 1) / image.shape[1] == 1).sum()
...
def extractor(region):
    ...
    return np.array([region.area / region.image.size, cx, cy, perimeter, holes, vlines, hlines, eccentricity, aspect])
